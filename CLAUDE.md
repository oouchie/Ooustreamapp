...
- **Current Version**: 3.6.3 (versionCode 60)
...
8. `LiveTvFragment.kt` — category/channel lists, debounced EPG loading, preview player, SmartEpgFiller for channel EPG text, MultiView icon. Preview panel (`previewContainer`) is deliberately **not** D-pad focusable (v3.6.4 pending) — making it focusable on channel click stole focus from the channel item and caused the cursor to disappear. Fullscreen flow is "second OK on same channel" only.
