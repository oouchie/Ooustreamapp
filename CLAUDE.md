...
- **Current Version**: 3.6.8 (versionCode 65)
...
- **Audio & Subtitle Track Picker** — Right-side slide-in panel for switching audio/subtitle tracks during playback. D-pad navigable, radio-button selection, always shows Default audio + Off subtitles as fallbacks. Triggered from "Tracks" button on controls bar. Unified across both backends as of v3.6.6: ExoPlayer uses `show(player: Player)` which goes through `refreshTracks → renderTracks`; libVLC uses `showCustom(audioTracks, subtitleTracks, onAudioSelect, onSubtitleSelect)` with pre-built `TrackInfo` lists (public data class) and selection callbacks that route to `MediaPlayer.setAudioTrack` / `setSpuTrack`. Same layout, focus, and dismissal code for both. (`player/TrackPickerOverlay.kt`, `res/layout/overlay_track_picker.xml`)
...
11. `player/TrackPickerOverlay.kt` — audio/subtitle track picker slide-in panel. Two entry points: `show(player: Player)` for ExoPlayer, `showCustom(audio, subs, onAudioSelect, onSubtitleSelect)` for libVLC / any non-ExoPlayer backend. Public `TrackInfo` data class carries either `(groupIndex, trackIndex)` (ExoPlayer) or `customId` (VLC's native int id). Shared `renderTracks()` builds the UI; `addTrackItem` dispatches to `applyTrackSelection` (Player path) or the custom callback based on whether `activePlayer` is set.
